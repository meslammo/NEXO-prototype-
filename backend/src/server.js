...
app.get('/catalog', async (req) => {
  const qs=req.query||{};
  const type=String(qs.type||'').trim().toLowerCase();
...
app.post('/admin/catalog/upsert',{preHandler:adminAuth},async(req,reply)=>{
  ...
  try{
    const result=await q(`INSERT INTO nexo.gifts(id,name,rarity,gems,tradeable,image,tagline,active,item_type,category,description,animation,market_visible,sort_order,tags,metadata)
      ...
      ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,rarity=EXCLUDED.rarity,gems=EXCLUDED.gems,tradeable=EXCLUDED.tradeable,
      image=EXCLUDED.image,tagline=EXCLUDED.tagline,active=EXCLUDED.active,item_type=EXCLUDED.item_type,category=EXCLUDED.category,
      description=EXCLUDED.description,animation=EXCLUDED.animation,market_visible=EXCLUDED.market_visible,sort_order=EXCLUDED.sort_order,
      tags=EXCLUDED.tags,metadata=EXCLUDED.metadata RETURNING id`,
    ...
    await adminAudit(req,'catalog_upsert',id,{name,itemType,rarity,gems});
...
app.post('/admin/catalog/:id/toggle',{preHandler:adminAuth},async(req,reply)=>{
  ...
  await adminAudit(req,'catalog_toggle',req.params.id,r.rows[0]); return r.rows[0];
});
...
app.post('/admin/users/:id/wallet',{preHandler:adminAuth},async(req,reply)=>{
  ...
  await adminAudit(req,'wallet_adjust',req.params.id,{gemsDelta,energyDelta}); return r.rows[0];
});
app.post('/admin/users/:id/status',{preHandler:adminAuth},async(req,reply)=>{
  const banned=Boolean((req.body||{}).banned);
  const r=await q('UPDATE nexo.users SET banned=$1 WHERE id=$2 RETURNING id,banned',[banned,req.params.id]);
  if(!r.rowCount)return reply.code(404).send({error:'USER_NOT_FOUND'});
  await adminAudit(req,banned?'ban_user':'unban_user',req.params.id,{banned}); return r.rows[0];
});
...
app.get('/admin/trades',{preHandler:adminAuth},async()=> (await q(`SELECT id,from_user_id AS "fromUserId",to_user_id AS "toUserId",status,from_items AS "fromItems",to_items AS "toItems",from_gems AS "fromGems",to_gems AS "toGems",fee_gems AS "feeGems",created_at AS "createdAt",expires_at AS "expiresAt"
...
app.post('/admin/announce',{preHandler:adminAuth},async(req,reply)=>{
  ...
  await adminAudit(req,'announce',null,{title,recipients:users.rowCount});
...
app.post('/gifts/buy',{preHandler:auth},async(req,reply)=>{
  ...
  try{
    return await tx(async c=>{
      ...
      if(!g.rowCount)throw Object.assign(new Error('ITEM_NOT_IN_MARKET'),{code:404});
...
+params.length);}
  if(category){params.push(category);clauses.push('category=app.get('/users/search', async (req, reply) => {
  const query = String((req.query || {}).q || '').trim().slice(0,40);
  if (!query) return [];
  return (await q(`SELECT u.id,u.username,u.display_name AS "displayName",u.avatar,u.name_color AS "nameColor",
    u.vip_level AS "vipLevel",COALESCE(p.online,false) AS online
...
app.post('/profile/equipped',{preHandler:auth},async(req,reply)=>{
  ...
  try{
    return await tx(async c=>{
      ...
      if(!own.rowCount||Number(own.rows[0].quantity)<1)return reply.code(409).send({error:'ITEM_NOT_OWNED'});
